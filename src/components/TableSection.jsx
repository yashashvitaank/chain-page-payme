  ...
  function Column({upperText, lowerText, trnTime}){
    return (
      <div className='flex flex-col'>
        <p className='font-bold'>{isBlock ? "Validated by" : "From"} <span style={{color:color, fontWeight: "normal"}}>{upperText}</span></p>
  ...
  function RenderRow({rowData}){
    ...
    return(
      <div className='flex justify-between px-3 w-full border-b border-zinc-200 py-2'>
        ...
        <img src={icon} />
        <div className='flex flex-col'>
          <Link to={`${redirectionUrl+rowData.id}`} style={{color: color, fontWeight: "bold"}}>{rowData.id}</Link>
